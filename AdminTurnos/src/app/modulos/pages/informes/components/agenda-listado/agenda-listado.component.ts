...
@Component({
  ...
  styleUrls: ['./agenda-listado.component.css'],
  animations: [
    trigger('openClose', [
      state('open', style({
        height: '*',
...
export class AgendaListadoComponent implements OnInit {
  ...
  public rut = new FormControl('');
  isSubmitted = false;
  validatedRut = false;
  isRutRequired: boolean = true;
  idOk: Boolean = false;
  constructor(
  ...
  async resumenAtencion() {
    ...
    if (this.form.valid) {
      ...
      const resp = await this.informeService.informeAtencionAgrupado(this.form.value)
  ...
  selecteIdOficina(event: any, oficinaId: number) {
    ...
    formatRut(rut: string): void {
    console.log(this.isRutRequired);
    if (this.isRutRequired) {
      this.rut.setValue(this.formatearRut(rut));
    }
  }
