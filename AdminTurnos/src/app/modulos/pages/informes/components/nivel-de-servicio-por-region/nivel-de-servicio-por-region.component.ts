...
import { AFCNivelServicioRegionalesService } from 'src/app/core/services/utils/excelService/afc-nivel-servicio-regionales.service';

@Component({
  selector: 'app-nivel-de-servicio-por-region',
  templateUrl: './nivel-de-servicio-por-region.component.html',
  styleUrls: ['./nivel-de-servicio-por-region.component.css'],
  animations: [
    trigger('openClose', [
      state('open', style({
        height: '*',
...
export class NivelDeServicioPorRegionComponent implements OnInit {
  ...
  constructor(
    ...
    private excelService: AFCNivelServicioRegionalesService
  ) {
  ...
  async controlPosicion(): Promise<any> {
    this.informe.sort((a:any, b:any) => {

    if (a.idX === null && b.idX !== null) return -1;
    if (a.idX !== null && b.idX === null) return 1;

    if (a.oficina === "TOTAL AFC") return -1;
    if (b.oficina === "TOTAL AFC") return 1;

    if (a.oficina === "Capitales Regionales") return -1;
    if (b.oficina === "Capitales Regionales") return 1;

    if (a.oficina === "Otras Sucursales") return -1;
    if (b.oficina === "Otras Sucursales") return 1;
