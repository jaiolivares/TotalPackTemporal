...
import { AFCPausasSucursalService } from 'src/app/core/services/utils/excelService/afc-pausas-sucursal.service';

@Component({
  selector: 'app-pausas-por-sucursal',
  templateUrl: './pausas-por-sucursal.component.html',
  styleUrls: ['./pausas-por-sucursal.component.css'],
  animations: [
    trigger('openClose', [
      state('open', style({
        height: '*',
...
export class PausasPorSucursalComponent implements OnInit {
  ...
  constructor(
    ...
    private excelService: AFCPausasSucursalService
  ) {
  ...
  async onsubmit() {
    ...
    if (this.form.valid) {
      ...
      const resp = await this.informeService.informePausasSucursal(this.form.value)
