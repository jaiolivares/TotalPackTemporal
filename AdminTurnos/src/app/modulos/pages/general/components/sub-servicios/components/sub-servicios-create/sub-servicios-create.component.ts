...
@Component({
  selector: 'app-sub-servicios-create',
  templateUrl: './sub-servicios-create.component.html',
  animations: [
    trigger('openClose', [
      state('open', style({
        height: '*',
...
export class SubServiciosCreateComponent implements OnInit {
  ...
  async onSubmit() {
    ...
    if (this.form.invalid) {
      ...
      try {
        const resp: any = await this.subSeriesService.agregarSubSerie(
          this.localService.getValue('customer').slug,
          data
        );
        if (resp['status'] && resp.data[0].codError == 0) {
          this.sweetAlertService.toastConfirm('success', 'El menú se ha agregado con éxito');
          this.volver(true);
        } else {
          this.sweetAlertService.toastConfirm('error', `Ha ocurrido un error al agregar el menú  ${resp.data?.[0]?.['msg'] ? '- ' + resp.data?.[0]?.['msg'] : ''}`);
        }
        this.spinner.hide("servicio-create-menu-loading");
      }
      catch (error: any) {
        this.spinner.hide("servicio-create-menu-loading");
        this.sweetAlertService.toastConfirm('error', 'Ha ocurrido un error al agregar el menú');
      }
    }
  }
  ...
  handleErrors() {
    ...
    this.sweetAlertService.toastConfirm('error', `No se ha podido crear el menú, debido a los siguientes errores:\n${mensajeErrores}`);
  }
