...
export class AFCAgendaListado {
  ...
  async download(dataExcel: any): Promise<void> {
    ...
    this._workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Reporte DWH Agenda Listado.xlsx');
    });
  }
  ...
  private async createtable(dataExcel: any) {
    ...
    const subtitleCell5 = sheet.getCell('E9');
    ...
    const subtitleCell6 = sheet.getCell('F9');
    ...
    const subtitleCell7 = sheet.getCell('G9');
    ...
    subtitleCell1.value = 'Oficina';
    subtitleCell2.value = 'Serie';
    subtitleCell3.value = 'Fecha';
    subtitleCell4.value = 'Hora';
    subtitleCell5.value = 'Rut';
    subtitleCell6.value = 'Teléfono';
    subtitleCell7.value = 'Mail';


    //agregar colores solo a las cabeceras
    ['A9', 'B9', 'C9', 'D9', 'E9', 'F9', 'G9'].forEach(key => {
      const cell = sheet.getCell(key);
      cell.fill = {
        type: 'pattern',
        pattern: 'solid',
