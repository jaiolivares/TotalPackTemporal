...
export class AtencionNoAgrupadoService {
  ...
  async download(dataExcel: any): Promise<void> {
    ...
    this._workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Reporte Atención no Agrupado.xlsx');
    });
  }
  ...
  private async createtable(dataExcel: any) {
    ...
    // Nombre del titulo
    titleCell.value = 'Reporte de Atención (no Agrupado)';
    ...
    cabeceras.values = ['Minutos', 'Normal', 'Especial', 'Total', '%', '#', '%', 'Prom Acumulado'];
    cabeceras.font = {
      name: 'Arial',
      size: 12,
      bold: true
    ...
    dataExcel.forEach((element: any) => {
      ...
      let rowValues = [];
      // Verifica si element.qAteTOL es un string y, de ser así, excluye los types number
      if (typeof element.qAteTOL === 'string') {
        rowValues = [
          element.rngtxt,
          element.qAteNOfi,
          element.qAteTOL, // Incluye qAteTOL solo si es string
          element.qAteE,
          element.qAteT,
          element.pAte,
          element.qTotA,
