...
export class InternetService {
  ...
  constructor(private router: Router, private httpClient: HttpClient, private cache: StorageService, private sweetAlertService: SweetAlertService) {}
  ...
  async checkApi(opensweet: boolean): Promise<boolean> {
    try {
      const response: GlobalResponse = await lastValueFrom(this.httpClient.get<GlobalResponse>(Settings.api_auxiliar.url, { headers: this.headers }));
